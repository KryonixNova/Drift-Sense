...
def test_resolve_preset_dram_1x_matches_afb_defaults():
    overrides = m.resolve_preset("dram_1x")
    assert overrides == {
...
def test_resolve_preset_unknown_name_raises_keyerror():
    with pytest.raises(KeyError):
        m.resolve_preset("dram_nonexistent")


@pytest.mark.parametrize("preset_name", [
    "dram_1x", "dram_dense", "dram_loose", "dram_wide", "dram_compact", "dram_legacy",
])
def test_preset_geometry_keeps_array_square(preset_name):
    overrides = m.resolve_preset(preset_name)
    p = m.DRAMParams(**overrides)
    array_w_nm = p.cols * p.cell_pitch_bl_nm
    array_h_nm = p.rows * p.cell_pitch_nm
    assert array_w_nm == pytest.approx(array_h_nm, rel=0.01)


def test_dram_params_new_zone_fields_default_passthrough():
    p = m.DRAMParams()
    assert p.block_presets is None
    assert p.boundary_bias == 0.0


def test_dram_generator_rejects_nonpositive_reference_width(tmp_path):
    p = m.DRAMParams(reference_width_px=0,
                      output_gds=str(tmp_path / "x.gds"), output_json=str(tmp_path / "x.json"))
    with pytest.raises(ValueError):
        m.DRAMGenerator(p)


def test_dram_generator_rejects_nonpositive_zoom_ratio(tmp_path):
    p = m.DRAMParams(zoom_ratio=0,
                      output_gds=str(tmp_path / "x.gds"), output_json=str(tmp_path / "x.json"))
    with pytest.raises(ValueError):
        m.DRAMGenerator(p)


def test_dram_generator_accepts_true_zoom_reference_size(tmp_path):
    p = m.DRAMParams(reference_width_px=100, reference_height_px=100, zoom_ratio=10,
                      output_gds=str(tmp_path / "x.gds"), output_json=str(tmp_path / "x.json"),
                      seed=1)
    m.DRAMGenerator(p)


def test_generate_sample_true_zoom_produces_1000px_reference(tmp_path):
    sample = m.generate_sample(seed=1, tmp_dir=str(tmp_path), zoom_ratio=10)
    assert sample.reference_img.shape[:2] == (1000, 1000)
    assert sample.zoom_ratio == 10


def test_generate_sample_default_zoom_ratio_unchanged(tmp_path):
    sample = m.generate_sample(seed=1, tmp_dir=str(tmp_path))
    assert sample.reference_img.shape[:2] == (100, 100)
    assert sample.zoom_ratio == 1
...
def test_dram_params_die_defaults_are_passthrough():
    ...
    assert p.die_block_width_nm == 5120.0
    assert p.die_block_height_nm == 5120.0
    assert p.die_street_width_nm == 100.0
...
def test_square_die_block_width_nm_reduces_to_height_for_square_grid():
    w = m.square_die_block_width_nm(rows=2, cols=2, street_width_nm=40.0,
                                     block_height_nm=5120.0)
    assert w == pytest.approx(5120.0)


def test_square_die_block_width_nm_makes_total_die_square_for_rectangular_grid():
    rows, cols, street, block_h = 2, 3, 40.0, 5120.0
    block_w = m.square_die_block_width_nm(rows, cols, street, block_h)
    total_w = cols * block_w + (cols - 1) * street
    total_h = rows * block_h + (rows - 1) * street
    assert total_w == pytest.approx(total_h)
...
def test_die_generator_block_presets_selects_from_list(tmp_path):
    p = m.DRAMParams(
        die_block_rows=1, die_block_cols=2,
        die_block_width_nm=800.0, die_block_height_nm=800.0,
        die_street_width_nm=30.0,
        block_presets=["dram_dense"],
        output_gds=str(tmp_path / "die.gds"),
        output_json=str(tmp_path / "die.json"),
        seed=21,
    )
    meta = m.DieGenerator(p).generate()
    dense = m.resolve_preset("dram_dense")
    frac = p.die_block_variation_frac
    for block in meta["blocks"]:
        cp = block["params"]["cell_pitch_nm"]
        base = dense["cell_pitch_nm"]
        assert base * (1 - frac) - 1e-6 <= cp <= base * (1 + frac) + 1e-6


def test_die_generator_block_presets_none_is_byte_identical_to_before(tmp_path):
    p = m.DRAMParams(
...
def test_die_generator_boundary_bias_one_straddles_a_street(tmp_path):
    for seed in range(5):
        p = m.DRAMParams(
            die_block_rows=2, die_block_cols=2,
            die_block_width_nm=800.0, die_block_height_nm=800.0,
            die_street_width_nm=30.0,
            boundary_bias=1.0,
            output_gds=str(tmp_path / f"die_{seed}.gds"),
            output_json=str(tmp_path / f"die_{seed}.json"),
            seed=seed,
        )
        meta = m.DieGenerator(p).generate()
        rx0, ry0, rx1, ry1 = meta["reference_bbox_nm"]
        v_street = rx1 > 800.0 and rx0 < 830.0
        h_street = ry1 > 800.0 and ry0 < 830.0
        assert v_street or h_street, f"seed {seed}: {meta['reference_bbox_nm']} doesn't straddle a street"


def test_die_generator_boundary_bias_zero_never_straddles_a_street(tmp_path):
    for seed in range(5):
        p = m.DRAMParams(
            die_block_rows=2, die_block_cols=2,
            die_block_width_nm=800.0, die_block_height_nm=800.0,
            die_street_width_nm=30.0,
            boundary_bias=0.0,
            output_gds=str(tmp_path / f"die0_{seed}.gds"),
            output_json=str(tmp_path / f"die0_{seed}.json"),
            seed=seed,
        )
        meta = m.DieGenerator(p).generate()
        rx0, ry0, rx1, ry1 = meta["reference_bbox_nm"]
        v_street = rx1 > 800.0 and rx0 < 830.0
        h_street = ry1 > 800.0 and ry0 < 830.0
        assert not (v_street or h_street), f"seed {seed}: unexpectedly straddles a street"


def test_die_generator_boundary_bias_noop_for_1x1_die(tmp_path):
    p = m.DRAMParams(rows=8, cols=8, boundary_bias=1.0,
                      output_gds=str(tmp_path / "d.gds"), output_json=str(tmp_path / "d.json"),
                      seed=5)
    meta = m.DieGenerator(p).generate()
    assert "reference_bbox_nm" in meta
