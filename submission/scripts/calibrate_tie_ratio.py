...
Sweeps tau in [0.80, 1.00] and selects the value maximising acc@Tpx (T set
via --best-metric, default 5), then reports the full sensitivity curve so
the choice is auditable rather than arbitrary.

Example:
    python scripts/calibrate_tie_ratio.py --checkpoint model/production_v3/best.pt
"""
...
def parse_args():
    ...
    p.add_argument("--best-metric", type=int, default=5, choices=[1, 3, 5],
                   help="which acc@Tpx tolerance (in px) selects the best "
                        "peak_tie_ratio from the sweep.")
...
def main():
    ...
    print(f"{'tau':>6} {best_key:>9} {'acc@1px':>9}")
    for tau in np.arange(0.80, 1.001, 0.01):
        ...
        print(f"{tau:>6.2f} {m[best_key]:>9.3f} {m['acc@1px']:>9.3f}")

    best = max(curve, key=lambda t: curve[t][best_key])
    print(f"\nselected peak_tie_ratio = {best} "
          f"({best_key} {curve[best][best_key]:.3f})")
