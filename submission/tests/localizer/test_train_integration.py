...
@pytest.mark.slow
def test_best_metric_persisted_and_resets_best_acc_on_change(tmp_path):
    import torch as _torch

    r1 = _run_train(tmp_path, ["--best-metric", "5"])
    assert r1.returncode == 0, r1.stderr
    ckpt1 = _torch.load(tmp_path / "geo_test" / "last.pt", weights_only=False)
    assert ckpt1["best_metric"] == 5

    r2 = _run_train(tmp_path, ["--best-metric", "1", "--resume"])
    assert r2.returncode == 0, r2.stderr
    assert "NOTE: profile changed" in r2.stdout
    assert "best_metric" in r2.stdout
    ckpt2 = _torch.load(tmp_path / "geo_test" / "last.pt", weights_only=False)
    assert ckpt2["best_metric"] == 1


def test_best_metric_rejects_values_outside_1_3_5(tmp_path):
    cmd = [_sys.executable, "scripts/train.py", "--run-name", "bad_metric_test",
           "--out-dir", str(tmp_path), "--max-steps", "2", "--best-metric", "50"]
    result = subprocess.run(cmd, capture_output=True, text=True, cwd=REPO_ROOT)
...
def test_resume_and_init_from_together_is_rejected(tmp_path):
    cmd = [_sys.executable, "scripts/train.py", "--run-name", "conflict_test",
           ...
           "--init-from", "model/production_v3/best.pt"]
    result = subprocess.run(cmd, capture_output=True, text=True, cwd=REPO_ROOT)
    assert result.returncode != 0
